...
class Sidebar(ctk.CTkFrame):
    def __init__(self, master, on_select):
        ...
        SidebarButton(self, "Dashboard", lambda: on_select("Dashboard")).pack(fill="x", padx=10, pady=5)
        SidebarButton(self, "Legal Q&A", lambda: on_select("Legal Q&A")).pack(fill="x", padx=10, pady=5)
        SidebarButton(self, "Bills",     lambda: on_select("Bills")).pack(fill="x", padx=10, pady=5)
...
class DashboardPanel(ctk.CTkFrame):
    def __init__(self, master, engine):
        ...
        ctk.CTkLabel(self, text="Platform Overview", font=("Inter", 24, "bold"),
                     text_color=PALETTE["text_primary"]).pack(anchor="w", padx=40, pady=(40, 20))

        stats_frame = ctk.CTkFrame(self, fg_color="transparent")
        stats_frame.pack(fill="x", padx=40)

        self._stat_card(stats_frame, "DATABASE RECORDS",  str(engine.total_records),        0)
        self._stat_card(stats_frame, "ACTIVE LIBRARIES",  str(len(engine.csv_files)),        1)
        self._stat_card(stats_frame, "AI STATUS",         "CONNECTED",                       2)
    ...
    def _stat_card(self, master, title, value, col):
        ...
        ctk.CTkLabel(card, text=title, font=("Inter", 10, "bold"),
                     text_color=PALETTE["text_secondary"]).pack(pady=(20, 5))
        ctk.CTkLabel(card, text=value, font=("Inter", 20, "bold"),
...
class LegalQAPanel(ctk.CTkFrame):
    def __init__(self, master, engine):
        ...
        self.input_field = ctk.CTkEntry(
            ...
            text_color=PALETTE["text_primary"], font=("Inter", 14)
        )
        ...
        self.submit_btn = ctk.CTkButton(
            ...
            text_color=PALETTE["bg"], font=("Inter", 12, "bold"),
    ...
    def _run_ai(self, query):
        try:
            ...
            self.after(0, self._append, f"EXECUTIVE INSIGHT: {response}\n\n")
        except Exception as e:
            self.after(0, self._append, f"SYSTEM ERROR: {e}\n\n")
        finally:
...
class BillsPanel(ctk.CTkFrame):
    def __init__(self, master, engine):
        ...
        ctk.CTkLabel(self, text="Legislative Tracking", font=("Inter", 24, "bold"),
                     text_color=PALETTE["text_primary"]).pack(anchor="w", padx=40, pady=(40, 20))
        ...
        self.search_entry = ctk.CTkEntry(
            search_frame, placeholder_text="Filter database by keywords (e.g. 'Sanctions', 'Space')...",
            fg_color="transparent", border_width=0
        )
        ...
        ctk.CTkButton(
            ...
            width=150, command=self.do_search
        ).pack(side="right", padx=10, pady=10)

        self.results_box = ctk.CTkTextbox(
            self, fg_color=PALETTE["surface"], border_color=PALETTE["border"],
            border_width=1, text_color=PALETTE["text_secondary"]
        )
...
class SettingsPanel(ctk.CTkFrame):
    def __init__(self, master):
        ...
        ctk.CTkLabel(self, text="Preferences", font=("Inter", 24, "bold"),
                     text_color=PALETTE["text_primary"]).pack(anchor="w", padx=40, pady=(40, 20))
        ...
        ctk.CTkLabel(container, text="Appearance Mode", font=("Inter", 14, "bold"),
        ...
        ctk.CTkLabel(container, text="AI Core Engine", font=("Inter", 14, "bold"),
                     text_color=PALETTE["text_primary"]).pack(anchor="w", padx=30, pady=(30, 10))
        ctk.CTkLabel(container, text="Connected via OpenAI API (gpt-3.5-turbo-instruct)",
                     text_color=PALETTE["positive"]).pack(anchor="w", padx=30)

        ctk.CTkLabel(container, text="Database", font=("Inter", 14, "bold"),
                     text_color=PALETTE["text_primary"]).pack(anchor="w", padx=30, pady=(30, 10))
        ctk.CTkButton(
            container, text="REBUILD DATABASE",
