...
Each panel contains:
  • A live Google search bar pre-loaded with a relevant query
  • A threaded scraper that fetches results without freezing the UI
  • The original static data displayed below as a permanent reference

Dependencies (install once):
    pip install requests beautifulsoup4 googlesearch-python

Usage — in your main file, replace the five panel class definitions
with this file's versions.  Everything else (PALETTE, tr, WARS_DATA,
ELECTION_DATA, ECON_DATA, CONGRESS_DATA, SCOTUS_DATA, GOV_ERAS, …)
...
def _google_search_results(query: str, num: int = 8) -> list[dict]:
    """
    Scrape Google search results for *query*.

    Returns a list of dicts:
        {"title": str, "url": str, "snippet": str}

    Falls back to an empty list on any error (network, block, parse).
    Strategy:
      1. Try the googlesearch-python library (cleanest).
      2. Fall back to scraping google.com/search directly.
    """
    results = []
    ...
    try:
        ...
        if results:
            # Enrich with page titles / snippets via a lightweight HEAD+GET
            enriched = []
...
def _make_scraper_section(
    parent_frame,
    default_query: str,
    palette: dict,
) -> tuple:
    """
    Build the reusable 'Live Google Search' widget block.

    Returns (section_frame, run_search_callback).
    The caller should call run_search_callback(query) to trigger a search.
    """
    outer = ctk.CTkFrame(
        parent_frame,
    ...
    # ── Wire up the search ────────────────────────────────────────────────
    def _run_search(query: str | None = None):
        ...
        def _render_results(data: list[dict]):
            for w in results_scroll.winfo_children():
                w.destroy()
            ...
            if not data:
                ctk.CTkLabel(
                    ...
                    text=(
                        ...
                        "  • Google blocked the request (try again in 60 s)\n"
...
def _make_result_card(parent, item: dict, palette: dict):
    """Render one Google result as a compact card."""
    card = ctk.CTkFrame(
        parent,
...
# ══════════════════════════════════════════════════════════════════════════════
#  US WARS PANEL  (with live scraper)
# ══════════════════════════════════════════════════════════════════════════════
...
class USWarsPanel(ctk.CTkFrame):
    ...
    def _build(self):
        ...
        # Live search widget
        _, auto_search = _make_scraper_section(
            outer_scroll,
        ...
        ctk.CTkLabel(
            ...
            text="⚠  Figures below are historically documented totals. "
                 ...
                 "figures will change as the conflict continues.",
            font=("Courier New", 10),
            text_color=PALETTE["text_dim"],
        ...
        self._cards: list[tuple[str, ctk.CTkFrame]] = []

        for label in ("All", "Ongoing", "Historical"):
            ctk.CTkButton(
                filter_row, text=label, width=90, height=26, corner_radius=6,
        ...
        # ── War cards (static data) ───────────────────────────────────────
        ctk.CTkLabel(
            outer_scroll,
            text="HISTORICAL & ONGOING CONFLICTS  —  STATIC DATABASE",
            font=("Courier New", 9, "bold"),
...
# ══════════════════════════════════════════════════════════════════════════════
#  ELECTION HISTORY PANEL  (with live scraper)
# ══════════════════════════════════════════════════════════════════════════════
...
class ElectionHistoryPanel(ctk.CTkFrame):
    ...
    def _build(self):
        ...
        ctk.CTkLabel(
            hdr, text="U.S. Presidential Election History",
            font=("Georgia", 24, "bold"),
            text_color=PALETTE["text_primary"], anchor="w",
        ).pack(side="left")
        ...
        for label in ("All", "Republican wins", "Democrat wins"):
            ctk.CTkButton(
                frow, text=label, width=130, height=26, corner_radius=6,
                fg_color=PALETTE["surface_2"], hover_color=PALETTE["border"],
        ...
        # Live search
        _make_scraper_section(
            scroll,
            default_query="US presidential election results 2024 news",
            palette=PALETTE,
        )

        # Static election cards
        ctk.CTkLabel(
            scroll,
            text="ELECTION HISTORY DATABASE",
            font=("Courier New", 9, "bold"),
            text_color=PALETTE["text_dim"], anchor="w",
        ).pack(fill="x", pady=(4, 8))

        self._election_cards: list[tuple[str, ElectionCard]] = []
...
# ══════════════════════════════════════════════════════════════════════════════
#  ECONOMY PANEL  (with live scraper)
# ══════════════════════════════════════════════════════════════════════════════
...
class EconomyPanel(ctk.CTkFrame):
    ...
    def _build(self):
        ...
        ctk.CTkLabel(
            hdr, text="U.S. Economic Dashboard",
            font=("Georgia", 24, "bold"),
            text_color=PALETTE["text_primary"], anchor="w",
        ).pack(side="left")
        ctk.CTkLabel(
            hdr, text=f"  as of {cur['as_of']}",
            font=("Courier New", 10),
            text_color=PALETTE["text_dim"],
        ...
        ctk.CTkLabel(
            scroll,
            text="ECONOMIC RECORD BY ADMINISTRATION",
            font=("Courier New", 9, "bold"),
            text_color=PALETTE["text_dim"], anchor="w",
        ).pack(fill="x", pady=(4, 8))
...
# ══════════════════════════════════════════════════════════════════════════════
#  CONGRESS TRACKER PANEL  (with live scraper)
# ══════════════════════════════════════════════════════════════════════════════
...
class CongressPanel(ctk.CTkFrame):
    ...
    def _build(self):
        ...
        ctk.CTkLabel(
            self,
            text="Search Google for live congressional news, or browse current composition & recent bills below.",
            font=("Courier New", 11),
            text_color=PALETTE["text_secondary"], anchor="w",
        ).pack(fill="x", padx=40, pady=(0, 12))
        ...
        for col_i, (chamber_name, data) in enumerate([
            ...
            ctk.CTkFrame(bar_bg, height=14, fg_color=PALETTE["rep_light"], corner_radius=4,
                         ).place(relx=1.0, rely=0, relwidth=r_seats / total,
            ...
            maj_color = (PALETTE["rep_light"] if data["majority"] == "Republican"
                         else PALETTE["dem_light"])
            ctk.CTkLabel(card, text=f"Majority: {data['majority']}",
                         font=("Courier New", 10, "bold"),
                         text_color=maj_color, anchor="w",
        ...
        for bill in cd["recent_bills"]:
            sc = PALETTE.get(STATUS_COLORS.get(bill["status"], "text_secondary"),
...
# ══════════════════════════════════════════════════════════════════════════════
#  SUPREME COURT PANEL  (with live scraper)
# ══════════════════════════════════════════════════════════════════════════════
...
class ScotusPanel(ctk.CTkFrame):
    ...
    def _build(self):
        ...
        for i, j in enumerate(SCOTUS_DATA["justices"]):
            jc = (PALETTE["rep_light"] if j["leaning"] == "Conservative"
                  else PALETTE["dem_light"])
            col_i = i % 3
            row_i = i // 3
            ...
            ctk.CTkLabel(card,
                         text=f"Appt: {j['appointed_by']} ({j['year']})  •  Age: {j['age']}",
        ...
        for r in SCOTUS_DATA["landmark_rulings"]:
            rc = (PALETTE["rep_light"] if r["leaning"] == "Conservative"
                  else PALETTE["dem_light"])
            imp_color = (PALETTE["danger"] if r["impact"] == "Major"
